...
class TestLegacyDoneWithMalformedStrings:
    ...
    def test_done_with_very_long_summary(self, tmp_path: Path) -> None:
        """Very long summary should not break verification."""
        log.init(RunDir.create(tmp_path, "long_summary"))
    ...
    def test_done_with_unicode_summary(self, tmp_path: Path) -> None:
        """Unicode in summary should work correctly."""
        log.init(RunDir.create(tmp_path, "unicode"))
        ...
        assert signal.called is True
        assert signal.summary == summary

    def test_done_with_newlines_and_special_chars(self, tmp_path: Path) -> None:
        """Summary with newlines and special chars should be preserved."""
        log.init(RunDir.create(tmp_path, "special_chars"))
